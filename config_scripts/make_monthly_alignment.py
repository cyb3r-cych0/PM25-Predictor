...
# Keys are target column names we want in the aligned dataframe.
INPUT_FILES = {
...
# Try a set of common datetime formats before falling back to pandas autodetect
COMMON_FORMATS = [
    "%Y-%m-%d %H:%M:%S",
    "%Y-%m-%d",
    "%d/%m/%Y %H:%M:%S",
...
def read_and_monthly(path: Path, colname: str):
    if not path.exists():
    ...
    parsed = try_parse_dates(df[date_col].astype(str))
...
def align_monthly(input_files: dict, how_outer=False):
    dfs = {}
    for key, p in input_files.items():
        print(f"Reading {key} <- {p.name}")
        df_month = read_and_monthly(Path(p), key)
        dfs[key] = df_month
...
def compute_overlap_matrix(dfs):
    keys = list(dfs.keys())
    n = len(keys)
...
def main():
    ...
    outer_df, dfs = align_monthly(INPUT_FILES, how_outer=True)
    outer_out = OUT_DIR / "aligned_monthly_outer.csv"
    ...
    inner_df, _ = align_monthly(INPUT_FILES, how_outer=False)
    inner_out = OUT_DIR / "aligned_monthly_inner.csv"
    ...
    # overlap matrix
    keys, mat = compute_overlap_matrix(dfs)
    ...
    rpt = OUT_DIR / "alignment_report.json"
